...
/**
 * ✅ Get the proper image URL for display
 * Works both locally & in production (EasyPanel, Hostinger, etc.)
 */
export function getImageUrl(imageUrl: string | null): string | null {
  if (!imageUrl) return null;

  // ❌ Skip emoji or invalid URLs
  if (
    imageUrl.length <= 2 &&
  ...
  const backendUrl =
    ...
    "https://backendparadise-backend.glwcvq.easypanel.host";

  // ✅ Handle relative upload paths correctly
  if (imageUrl.startsWith("/uploads/") || imageUrl.startsWith("uploads/")) {
    return `${backendUrl}${imageUrl.startsWith("/") ? "" : "/"}${imageUrl}`;
  }

  // ✅ Default fallback if something weird is passed
